...
Let's create a simple project with Next.js to demonstrate how it works. You will be surprised how little we need to do in order to start our actual application. Some complexity is hidden by the library and the rule convention over configuration is helping to keep the code on our side really slim.
...
And if you don't like styled-jsx you can always switch to sass, inline-css, css-in-js with [https://github.com/zeit/next.js/#importing-css--sass--less--stylus-files](just a little configuration).
...
Next.js is by all means [https://nextjs.org/showcase/](production ready) and would fit for enterprise application and not only some small projects. It is built in a way that enables you to access lower level parts when needed, which makes it extremely flexible in what it does.
Not only is it really easy to use - it also has an [https://nextjs.org/docs](amazing documentation). So far, every question we had was answered in their FAQ or through examples provided in their repository.
